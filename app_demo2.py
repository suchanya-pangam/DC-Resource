...
.satellite-preview { display: grid; grid-template-columns: 1fr 320px; gap: 16px; margin: 16px 0; align-items: start; }
.preview-tile { background: linear-gradient(180deg, rgba(59,130,246,0.10), rgba(14,165,233,0.08)); border-radius: 20px; overflow: hidden; display: grid; grid-template-rows: 1fr auto; border: 1px solid rgba(15,23,42,0.08); min-height: 220px; }
.tile-swatch { display: flex; align-items: center; justify-content: center; min-height: 140px; padding: 18px; background: rgba(15,23,42,0.95); color: #fff; }
.tile-swatch-inner { width: 100%; text-align: center; }
.tile-caption { padding: 14px 12px 16px; background: #fff; display:flex; flex-direction:column; gap:8px; }
...
.satellite-metric-grid { display: grid; grid-template-columns: repeat(2, minmax(140px, 1fr)); gap: 12px; align-items: start; }
.metric-card { background: #fff; border: 1px solid #E2E8F0; border-radius: 14px; display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; box-shadow: 0 12px 30px rgba(15,23,42,0.03); min-height:48px; }
.metric-label { font-size: 12px; color: #64748B; display:block; }
...
<div class="app">
  <aside class="sidebar">
    ...
    <div class="brand-sub">Environmental Monitoring<br/>UI Demo</div>
    <div class="nav">
      ...
      <button class="nav-btn" onclick="switchPage('alerts', this)">🔔 Demo Alerts</button>
    ...
    <div class="source-box">
      <b>Data Source</b><br/>
      Demonstration data workflow<br/>
      Google Earth Engine (project materials)<br/>
      (2020 – 2026)
    </div>
  </aside>
  ...
  <main class="main">
    <div class="header">
      <div class="header-text">
        ...
        <p>Demo prototype for environmental monitoring of data centers across the United States</p>
      </div>
  ...
  <section class="right-panel">
    ...
    <div class="section-divider"><span>🔔 Demo Alert Scenario</span></div>
    
    <div class="sub-grid">
      <div class="info-block" id="alertLevelCard"></div>
      <div class="info-block">
        <div class="info-block-title">Illustrative Pattern</div>
        <b>Example Heat & Water-Stress Trend</b><br/>
        <span style="color:var(--muted); font-size:10.5px;">Example wording that shows how a pattern could be displayed in this UI demo.</span>
      </div>
    </div>

    <div class="sub-grid">
      <div class="info-block">
        <div class="info-block-title">How to read this demo</div>
        <span style="color:var(--muted);">This prototype demonstrates how environmental context could be summarised for later review. It does not issue operational findings.</span>
      </div>
      <div class="info-block">
        <div class="info-block-title">Illustrative Follow-up</div>
        <span style="font-size:10px; line-height:1.4;">
          ✔️ Review the underlying data<br/>
          ✔️ Compare the selected period<br/>
          ✔️ Consider local context<br/>
          ✔️ Ask the responsible team to validate findings
        </span>
      </div>
    </div>
    ...
    <div class="sub-grid">
      <div class="info-block">
        <div class="info-block-title">Example Resources</div>
        <span style="color:var(--blue); font-weight:500; cursor:pointer;">
          💧 Local water information<br/>
          🌡️ Heat-safety guidance<br/>
          📋 Field-review process
        </span>
      </div>
      <div class="info-block">
        <div class="info-block-title">Email UI Example</div>
        <span style="color:var(--muted);">This demo does not collect email addresses or send notifications.</span>
        <div class="email-box">
          <input class="email-input" placeholder="No email is collected" disabled/>
          <button class="email-btn" disabled>UI Demo</button>
        </div>
      </div>
    </div>

    <div class="footer-bar">
      <span>🛡️ DC-Resource Intelligence Platform UI Demo</span>
      <span>Demonstration build: 2026</span>
    </div>
  </section>
  ...
  <main class="page-layer" id="pageLayer">
    <section id="page-hotspot" class="page">
      <div class="page-header">
        ...
        <button class="btn-primary">⬇️ Export Preview (UI Demo)</button>
      </div>
      ...
      <div class="hotspot-grid">
        ...
        <div style="display: flex; flex-direction: column; gap: 16px;">
          ...
          <div class="card" style="background: #FFF5F5; border-color: #FEE4E4;">
            <h3 style="margin-top:0; color: var(--red);">Demo Insight</h3>
            <p style="font-size:13px; line-height:1.5;">Illustrative text for the UI demo. It shows how environmental patterns could be summarised for further review.</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section id="page-alerts" class="page">
      <div class="page-header">
        <div class="page-title">
          <h1>Demo Alerts</h1>
          <p>Illustrative alert scenarios for UI demonstration only</p>
        </div>
      </div>
      <div class="card">
        <table class="alerts-table">
          <thead><tr><th>Date</th><th>Location</th><th>Risk Level</th><th>Demo Summary</th><th>Display Status</th><th>Preview</th></tr></thead>
          <tbody>
            <tr class="alert-row"><td>Oct 12, 2026</td><td><strong>US Region 01</strong></td><td><span class="badge Critical">Critical</span></td><td>Example of a heat-pattern summary for the UI demo.</td><td><span class="badge Pending">Sample</span></td><td><span style="color:var(--blue); font-weight:600;">View ↗</span></td></tr>
            <tr class="alert-row"><td>Oct 10, 2026</td><td><strong>US Region 02</strong></td><td><span class="badge Critical">Critical</span></td><td>Example of a water-stress summary for the UI demo.</td><td><span class="badge Pending">Sample</span></td><td><span style="color:var(--blue); font-weight:600;">View ↗</span></td></tr>
            <tr class="alert-row"><td>Sep 28, 2026</td><td><strong>US Region 03</strong></td><td><span class="badge Resolved">Stable</span></td><td>Example of a normalising-pattern summary for the UI demo.</td><td><span class="badge Resolved">Sample</span></td><td><span style="color:var(--muted);">Archived</span></td></tr>
          </tbody>
        </table>
      </div>
    </section>
    ...
    <section id="page-datacenters" class="page">
      <div class="page-header">
        <div class="page-title">
          ...
          <p>Illustrative directory for the UI demo</p>
        </div>
        <input type="text" placeholder="🔍 Search demo location..." style="padding:10px 16px; border:1px solid var(--line); border-radius:8px; width:250px;">
      </div>
      <div class="dc-grid">
        <div class="card"><div class="dc-card-header"><strong>US Region 01</strong><span class="badge Critical">ECI: 72</span></div><p style="font-size:12px; color:var(--muted); margin:0 0 16px 0;">Approximate demonstration region</p><button class="btn-primary" style="width:100%; background:var(--softBlue); color:var(--blue);" onclick="switchPage('hotspot', document.querySelectorAll('.nav-btn')[1])">Open Demo View</button></div>
        <div class="card"><div class="dc-card-header"><strong>US Region 02</strong><span class="badge Critical">ECI: 68</span></div><p style="font-size:12px; color:var(--muted); margin:0 0 16px 0;">Approximate demonstration region</p><button class="btn-primary" style="width:100%; background:var(--softBlue); color:var(--blue);" onclick="switchPage('hotspot', document.querySelectorAll('.nav-btn')[1])">Open Demo View</button></div>
        <div class="card"><div class="dc-card-header"><strong>US Region 03</strong><span class="badge Stable">ECI: 24</span></div><p style="font-size:12px; color:var(--muted); margin:0 0 16px 0;">Approximate demonstration region</p><button class="btn-primary" style="width:100%; background:var(--softBlue); color:var(--blue);" onclick="switchPage('hotspot', document.querySelectorAll('.nav-btn')[1])">Open Demo View</button></div>
      </div>
    </section>
    ...
    <section id="page-reports" class="page">
      <div class="page-header">
        <div class="page-title">
          ...
          <p>Illustrative report layout for the UI demo</p>
        </div>
      </div>
      <div class="card report-form">
        <div class="input-group"><label>Select Location</label><select><option>All Demo Locations</option><option>US Region 01</option></select></div>
        ...
        <div class="input-group"><label>Include Metrics</label><div style="display:flex; gap:16px; font-size:13px; flex-wrap:wrap;"><label><input type="checkbox" checked> ECI & ESS Scores</label><label><input type="checkbox" checked> Demo Insights</label><label><input type="checkbox"> Sample Data Links</label></div></div>
        <button class="btn-primary" style="margin-top:16px; width:200px;">Preview Report (UI Demo)</button>
      </div>
    </section>
    ...
    <section id="page-about" class="page">
      ...
      <div class="card" style="line-height:1.6; max-width:800px;">
        <h3>How it works</h3>
        <p>This demo uses prepared satellite-derived indicators and scoring outputs. The <strong>Environmental Change Index (ECI)</strong> highlights relative environmental change over time, while the <strong>Environmental Stress Score (ESS)</strong> summarises current environmental stress.</p>
...
function renderHotspotSection(currentData) {
  ...
  document.getElementById('aiAlertBox').innerHTML = `
    ...
    <div class="alert-banner-text">
      <b>Demo Alert Summary (${cleanName(activeRec.data_center_name)})</b><br/>
      Illustrative text showing how a change in environmental markers could be summarised. This is not an operational alert or recommendation.
    </div>
    <div>
  ...
  document.getElementById('alertLevelCard').innerHTML = `
    <div class="info-block-title">Demo Display Status</div>
